...
function init() {
  ...
  $("#guardar_registro_cp").on("click", function (e) { if ( $(this).hasClass('send-data')==false) { $("#submit-form-cp").submit(); } else { toastr_warning("Espera", "Procesando Datos", 3000); } });
  ...
  lista_select2("../ajax/cliente.php?op=select2_centroPoblado", '#idcentropoblado', null);
...
function templateCentroPoblando (state) {
  //console.log(state);
  if (!state.id) { return state.text; } 
  var $state = $(`<span class="fs-11" > ${state.text}</span><span class="fs-9" > (${state.title})</span>`);
  return $state;
...
function mostrar_cliente(idpersona_cliente) {
  ...
  $.post("../ajax/cliente.php?op=mostrar_cliente", { idpersona_cliente: idpersona_cliente }, function (e, status) {
    ...
    if (e.status) {
      ...
      $("#idcentropoblado").val(e.data.idcentro_poblado).trigger("change");
...
function modal_add_centro_poblado() {
  ...
  $.ajax({
    ...
    success: function (e) {
      ...
      if (e.status == true) {
        ...
        lista_select2("../ajax/cliente.php?op=select2_centroPoblado", '#idcentropoblado', e.data); 
			}else{
				ver_errores(e);
			}
...
function llenar_dep_prov_cp(input) {
...
$(function () {
  ...
  $('#idcentropoblado').on('change', function () { $(this).trigger('blur'); });
  ...
  $("#form-agregar-cliente").validate({
    ...
    messages: {
      ...
      idcentropoblado:  { required: "Campo requerido.", },      
  ...
  $('#idcentropoblado').rules('add', { required: true, messages: { required: "Campo requerido" } });
...
function reload_select(r_text) {
  ...
  switch (r_text) {
    ...
    case 'centroPbl':
      lista_select2("../ajax/cliente.php?op=select2_centroPoblado", '#idcentropoblado', null, '.charge_idctroPbl');
    break;    
